...
### 其他问题
重写的方法里需要用到父类的似有变量mScrollEnabled，之前我都是在当前类再声明一个同名的变量去取用，也不用写反射了，虽然没有什么问题，后来发现java继承类中同名的变量会在实例上产生多个值，以类名.变量的形式作为命名空间，通过反射可以打印出名称，是拼接的出来的，最后子类上的变量才是实例访问的这个变量，这里会有什么问题的，问题就出在父类和子类操作的不是同一个变量，有多个继承，如果都有同名变量那么调用他们方法中所访问的这个同名变量都是各自的，所以还是不要出现同名变量的好，取不到老老实实用反射去获取

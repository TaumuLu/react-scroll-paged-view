...
import ViewPagedPlatform from './view-paged'
...
type Listener = (params: { value: number }) => void

// @Connect
export default class ViewPaged extends ViewPagedPlatform {
  _lastPos: number

  _posListener: Set<Listener>

  _autoPlayTimer: number

  currentPage: number

  _addIndexs: number[]

  _touchSartTime: number
  ...
  _onChange(_posPage = this._posPage, isDiff = false, isOnChange = true) {
    ...
    if (!isDiff || size(loadIndex) !== size(oldLoadIndex)) {
      this.setState({ loadIndex }, () => {
        if (isOnChange) {
          onChange && onChange(this.currentPage, this._prevPage)
        }
      })
    } else {
      if (isOnChange) {
